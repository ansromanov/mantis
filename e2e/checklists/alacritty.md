# Mantis Manual Verification Checklist — Alacritty

Use this checklist to manually verify features and keybindings on **Alacritty**.
...
## System Info
* **OS Version**: (macOS / Linux / Windows version)
* **Alacritty Version**: 
* **Shell**: (e.g. zsh, bash, cmd, powershell)
...
| [ ] | **Bundled Language Plugins** | Press `p` in the tree to open the plugin manager, enable `rust`, `python`, and/or `go` (disabled by default), close with `Esc`. | Plugins toggle on/off; opening a `.rs`/`.py`/`.go` file and pressing `Space` on a brace/indentation block folds it. | |
