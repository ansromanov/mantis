...
/// The static registry of all available commands. Every entry has a unique
/// `action_id` used by the key handler to dispatch the right App method.
...
/// Fuzzy-filterable list of all registered commands.
...
impl CommandPalette {
    ...
    fn refilter(&mut self) {
        ...
        let hay = |i: usize| format!("{} {}", COMMANDS[i].name, COMMANDS[i].keybinding);
