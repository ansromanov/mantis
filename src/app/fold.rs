//! Fold state management on `App`.
//!
//! Bridges pure fold-region detection (crate-level [`fold`](crate::fold) and
//! [`yaml_fold`](crate::yaml_fold) modules) to the live content pane. Tracks
//! which regions are collapsed (`folded`), computes the fold-gutter width, maps
//! a physical line to its fold region, and rebuilds `fold_display_map` — the
//! display-line to physical-line table the renderer and scroll math consult
...
//!
//! Fold regions can come from the built-in YAML detector (for `.yaml`/`.yml`
//! files) or from a plugin via the `set_fold_regions` action.
...
impl App {
    ...
    /// Applies plugin-provided fold regions for `path`, if any exist, replacing
    /// the current regions. No-op when no plugin has registered regions for the
    /// path.
