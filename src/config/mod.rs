...
const DEFAULT_CONFIG_TEMPLATE: &str = r##"# tree-viewer configuration
# To open this file for editing from within tv: Ctrl+P → "Open config in editor"
...
scroll_percentage = true
in_file_search = true
# search_context_lines = 0  # lines of trailing context shown after each content match
# keep_search_query = false  # restore last search query when reopening search

# Git mode: show only changed files and their working-tree diffs.
# git_mode = false       # start in git mode
# git_mode_flat = false  # start git mode in flat/plain list view

# Keybindings. Each action accepts a list of key specs such as "q", "ctrl+c",
# "alt+.", "PageUp". Modifiers: ctrl, alt, cmd/super.
[keys]
quit = ["q", "ctrl+c"]
help = ["?"]
toggle_hidden = ["alt+."]
...
# Full theme files are in ~/.config/tree-viewer/themes/ — copy and edit one
