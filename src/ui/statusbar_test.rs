...
fn narrow5_shows_empty_bar() {
    ...
    // At width 5 even the version string (" v0.7.10", 8 chars) can't fit.
...
fn narrow20_keeps_version_drops_hint() {
    ...
    assert!(text.contains("v0.7.10"), "version should fit at width 20");
    assert!(
        !text.contains("j/k"),
...
fn narrow_keeps_errors_over_badges() {
    ...
    app.show_hidden = true;
    app.walk_errors = 3;
    // Version " v0.7.10" (8) + " [!3]" (5) = 13; " [hidden]" (9) = 22 total.
    ...
    assert!(text.contains("v0.7.10"), "version should be kept");
...
fn narrow_keeps_git_over_badges() {
    ...
    assert!(text.contains("v0.7.10"), "version kept");
