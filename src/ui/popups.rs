...
pub(super) fn draw_blame_panel(f: &mut Frame, app: &App, area: Rect) {
    let theme = &app.theme;
    ...
    let by_line: std::collections::HashMap<u32, &crate::git::BlameLine> =
...
mod tests {
    ...
    fn draw_blame_panel_noop_without_visual_line() {
        ...
        let backend = TestBackend::new(80, 30);
        let mut terminal = Terminal::new(backend).unwrap();
        // No visual-line selection: must render nothing and not panic.
        terminal
            .draw(|f| draw_blame_panel(f, &app, f.area()))
            .unwrap();
    }
