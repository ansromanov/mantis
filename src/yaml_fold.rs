...
//! the same or a shallower level. `detect_fold_regions` produces [`FoldRegion`]s
//! (the type is defined in [`fold`](crate::fold)). Counts YAML anchors and
//! aliases for the file-info display.
//!
//! This module knows nothing about `App` or rendering; `app::fold` adapts it to
//! live editor state and the UI consumes the resulting map for scrolling and the
//! fold gutter. The generalized [`fold`](crate::fold) module provides
//! [`build_display_map`](crate::fold::build_display_map) which converts a set of
//! collapsed regions into a display-line to physical-line lookup table.
