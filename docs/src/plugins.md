...
| k8s | `k8s` | Registers as a language provider for `.yaml`/`.yml` files with the `status_facts` capability — coexists with the `yaml` plugin's `fold` registration on the same extensions since they declare different capabilities. On file open, reports the first Kubernetes resource's identity (`Kind/name (namespace)`) plus per-kind counts across `---`-separated documents; after the debounced `on_content_cursor_change` event, it reports the resource containing the cursor. Files without both `apiVersion` and `kind` show no facts. |
