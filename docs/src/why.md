...
There are a lot of ways to look at code. So why `tv`?

Because `tv` is built for **one job** — moving through a codebase and reading it,
...
> 💡 **The short version:** reach for `tv` when you want to *explore a repo, read
...
`tv` gives you out of the box — a file tree, a fuzzy finder, full-text search,
...
`tv` skips all of that. There's no `init.lua`, no plugin manager, and no modes —
just arrow keys (or `hjkl` if you prefer) and your mouse. Everything listed above
already works the moment you run it.

> 🧭 Love Vim? Keep it. `tv` is the *browser* you open first; `e` hands the file
...
`tv` is a tiny native binary. It opens **instantly**, sips memory, and runs
...
## When tree-viewer is the right tool
...
For heavy, sustained editing — refactors, LSP, debugging — use your editor. `tv`
